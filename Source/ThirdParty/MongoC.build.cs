...
public class MongoC : ModuleRules
{
	...
	public MongoC(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Win64)
		{
			...
			PublicAdditionalLibraries.Add(LibsDir + "/bson-1.0.lib");
			PublicAdditionalLibraries.Add(LibsDir + "/mongoc-1.0.lib");
			...
			CopyToBinaries(Path.Combine(BinDir, "bson-1.0.dll"), Target);
			CopyToBinaries(Path.Combine(BinDir, "mongoc-1.0.dll"), Target);
		}
		else if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			...
			PublicSystemLibraryPaths.Add(LibsDir);
